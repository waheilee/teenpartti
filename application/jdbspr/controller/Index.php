...
//允许携带证书式访问（携带cookie）
    ...
    public function createuser()
    {
        try {
            ...
            if (config('is_jdbspr_trans') == 1) {
                $post_param['url'] = $gameURL;
                $res = $this->curl(config('trans_url_other').'/jdbspr/index/createuser',$post_param);
            } else {
    ...
    public function index(){
        try {
            ...
            save_log('jdbspr', '==='.request()->url().'===接口请求数据===' . $params);
            ...
            if ($action == 9) {
                ...
                $socket = new QuerySocket();
                $state = $socket->downScore($user_id, $gamemoney, $reference,39400);
                if ($state['iResult']!=0) {
                    $respons = [
                        "status"=>"9999",
                        'balance'=>$balance,
                ...
                //记录订单是否执行，防止重复
                Redis::set('jdbspr_is_exec_bet_'.$user_id.$reference,1,86400);
                $left_balance = $this->getSocketBalance($user_id);
                $respons = [
                    "status"=>"0000",
                    'balance'=>$left_balance,
            ...
            if ($action == 10) {
                ...
                $gameId = $params['mType'];
                $transaction_id = $reference = $params['transferId'];
                $refTransferIds = $params['refTransferIds'];
                $balance = $this->getSocketBalance($user_id);
                if (Redis::get('jdbspr_is_exec_result_'.$user_id.$reference)) {
                    $respons = [
                        "status"=>"9999",
