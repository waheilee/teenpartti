    ...
    public function GetGoldRanklist(): array
    {
        ...
        if(config('is_dmrateset') == '1'){
            $result = $this->getTableObject('[View_Accountinfo](NOLOCK)')->alias('a')
                ...
                ->where($where)
                ->field('AccountID,Money,OperatorId,TotalDeposit,TotalRollOut,RegisterTime,b.FreezonMoney as iFreezonMoney,e.CurWageRequire as asd,e.NeedWageRequire as dsa,CAST WHEN CAST(e.CurWageRequire AS FLOAT)=0 OR CAST(e.NeedWageRequire AS FLOAT)=0 THEN 0 ELSE CAST(e.CurWageRequire AS FLOAT) / CAST(e.NeedWageRequire AS FLOAT) as percentage,ISNULL(c.value,0) as win_dmrateset,ISNULL(d.value,0) as lose_dmrateset')
