...
//    }
    public function TViewUserDrawBack(&$Channel)
    {
        ...
        $data = (new BankDB())->getTableObject('View_UserDrawBack')->alias('a')
            ...
            ->join('(SELECT SUM(Money) as money , RoleId FROM [OM_GameOC].[dbo].[T_GMSendMoney]  WHERE OperateType IN (1,2) AND status=1 GROUP BY RoleId) d', 'd.RoleID=a.AccountID', 'left')
            ->join('(SELECT SUM(Money) as money , RoleId FROM [OM_GameOC].[dbo].[T_GMSendMoney] WHERE OperateType IN (3,4) AND status=1 GROUP BY RoleId) e', 'e.RoleID=a.AccountID', 'left')
            ->field('a.*,b.Money,c.CurWageRequire,ISNULL(d.money,0) as gameUp,ISNULL(e.money,0) as commissionUp')
            ->where('1=1 ' . $where)
            ->order($order)
            ->paginate($limit)
            ->toArray();
