...
class Pggame extends Main
{
    ...
    public function indexUser()
    {
        ...
        if (input('action') == 'list') {
            ...
            //取交集
            $ids = array_keys($data);
            $uids = (new \app\model\AccountDB())->getTableObject('T_Accounts')->where(['OperatorId'=>session('merchant_OperatorId')])->where('AccountID','in',implode($ids, ','))->column('AccountID',"AccountID");
    ...
    public function editUser()
    {
        ...
        if ($this->request->method() == 'POST') {
            $uid      = request()->param('uid');
            $RoleInfo    = (new \app\model\AccountDB())->getTableObject('T_Accounts')->where(['AccountID'=>$uid,'OperatorId'=>session('merchant_OperatorId')])->find();
            if(empty($RoleInfo)){
