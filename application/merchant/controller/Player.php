...
class Player extends Main
{
    ...
    public function onlineList()
    {
        ...
        $field = "AccountID ID,MachineCode,Mobile,countryCode,AccountName,Locked,LoginName,GmType,RegisterTime,LastLoginIP,LastLoginTime,TotalDeposit,TotalRollOut,Money,UserName,BankName,BankCardNo,IFSCCode,UPICode,RegIP,b.CtrlRatio ctrlratio,b.InitialPersonMoney,TigerCtrlValue,PersonCtrlMoney,(TotalRollOut-TotalDeposit) as GameProfit,ParentID,ParentIds";
        $db = new UserDB();
        ...
        $online_list['count'] = $online_list['total'];
...
//            $user['PayOut'] = ConverMoney($user['PayOut']);
            ...
            //佣金上分
            $commisionsf = $gameOCDB->getTableObject('T_GMSendMoney')->where('RoleId',$roleId)->where('status',1)->where('OperateType','in','3')->sum('Money')?:0;

            //佣金
            $commision = (new UserDB())->getTableObject('T_ProxyCollectData')->alias('b')->where('ProxyId', $roleId)->field('(b.Lv1Running*'.config('agent_running_parent_rate')[1].'+b.Lv2Running*'.config('agent_running_parent_rate')[2].'+b.Lv3Running*'.config('agent_running_parent_rate')[3].') as runningProfit,(b.Lv1Tax*0.3+b.Lv2Tax*0.09+b.Lv3Tax*0.027) as taxProfit')->find();
            if (app_name == 'runing') {
                $TotalProfit = $commision['runningProfit'] ?: 0;
            }
            if (app_name == 'tax') {
                $TotalProfit = $commision['taxProfit'] ?: 0;
            }
            $TotalProfit = (new DataChangelogsDB())->getTableObject('T_ProxyBonusLog')->where('RoleID',$roleId)->where('BonusType','in','3,4,5,8')->sum('BonusAmount')?:0;
            ConVerMoney($TotalProfit);
            $TotalProfit = bcadd($commisionsf, $TotalProfit,2);
            ...
            $proxyinfo['TotalProfit'] = $TotalProfit;
            $proxy_switech = 1;
...
//            $nickname = trim(input('nickname'));
            ...
            $packID = session('merchant_OperatorId');
            $mobile = input('mobile', '');
            $ip = input('ip', '');
...
//            if (!empty($nickname)) $where .= " AND nickname like '%$nickname%' ";
        ...
        $field = "AccountID ID,MachineCode,Mobile,countryCode,AccountName,Locked,LoginName,GmType,RegisterTime,LastLoginIP,LastLoginTime,TotalDeposit,TotalRollOut,Money,RegIP,VipLv,SystemRight,ParentIds,ParentID,ProxyChannelId";
        $db = new UserDB();
        //$userProxyInfo = new UserProxyInfo();
        $ProxyChannelConfig = (new GameOCDB())->getTableObject('T_ProxyChannelConfig')->column('*', 'ProxyChannelId');
        $ProxyChannel = (new GameOCDB())->getTableList('T_ProxyChannelConfig', '', 1, 1000, '');
        switch (input('Action')) {
            case 'list':
            // var_dump($where);die();
            ...
            case 'onlinelist':
                ...
                break;
            case 'exec':
                $field = "AccountID ID,MachineCode,Mobile,AccountName,LoginName,RegisterTime,LastLoginIP,TotalDeposit,TotalRollOut,Money";
                $result = $db->TViewAccount()->GetPage($where, "$orderby $ordertype", $field);
                foreach ($result['list'] as &$item) {
                    ConVerMoney($item['Money']);
...
//                    halt($rows[0]);
        ...
        $this->assign('selectData', $gameRoomList);
        return $this->fetch();
    }
        ...
        switch (input('Action')) {
            ...
            case  'add':
                if (request()->isAjax()) {
                    ...
                    $Operator = (new GameOCDB())->getTableObject('T_OperatorSubAccount')->where(['OperatorId'=>session('merchant_OperatorId')])->find();
                   
                    if (md5($password) != $Operator['PassWord']) {
                    ...
                    $userinfo = (new AccountDB())->getTableObject('T_Accounts')->where(['AccountID'=>$roleID,'OperatorId'=>session('merchant_OperatorId')])->find();
                    if (!$userinfo) {
                        return $this->error('玩家不存在或非本渠道玩家!');
                    }
                    ...
                    //额度
                    $ed = (new GameOCDB())->getTableObject('T_OperatorQuotaManage')->where('OperatorId',session('merchant_OperatorId'))->find()?:[];
                    //测试
                    if (strlen($roleID) == 7) {
                    ...
                    $res = (new GameOCDB())->getTableObject('T_OperatorQuotaManage')->where('OperatorId',session('merchant_OperatorId'))->data([$used_ed_name=>($used_ed + abs($money))])->update();
                    $row = $db->GMSendMoneyAdd(['RoleId' => $roleID, 'Money' => $money, 'status' => 0, 'Note' => $descript, 'checkUser' => "operator:".session('merchant_OperatorId'), 'OperateType' => $operatetype]);
                    if ($row > 0) {
            ...
            case 'send':
                if (request()->isAjax()) {
                    ...
                    $db = new  GameOCDB('',true);
                    $data = $db->TGMSendMoney()->GetRow("ID=" . $gmid);
                    
                    if (substr($data['checkUser'], 0,15) != "operator:".session('merchant_OperatorId')) {
                    ...
                    if ($data['OperateType'] == 1) {
                        ...
                        // try {
                        //     //判断余额
                        //     $commission = (new UserDB())->getTableObject('T_UserGameWealth')->where('RoleID',$data['RoleId'])->value('ProxyBonus')?:0;
                        //     $commission = $commission/bl;
                        //     if ($commission < $data['Money']) {
                        //         $row = $db->TGMSendMoney()->UPData(["status" => 2, "UpdateTime" => date('Y-m-d H:i:s')], "ID=" . $data['ID']);
                        //         if ($row > 0) return $this->success("佣金不足，已拒绝");
                        //     } else {
            ...
            case 'deny':
                ...
                $res = (new GameOCDB())->getTableObject('T_OperatorQuotaManage')->where('OperatorId',session('merchant_OperatorId'))->data([$used_ed_name=>($used_ed - $data['Money'])])->update();
...
//            var_dump($roleId );die;
    ...
    public function getRoleWage()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if ($roleid > 0) {
                ...
                foreach ($list as $k => &$v) {
                    $v['ChangeMoney'] = FormatMoneyint($v['ChangeMoney']);
                    $v['NeedWageRequire'] = FormatMoneyint($v['NeedWageRequire']);
                    $v['CurWageRequire'] = FormatMoneyint($v['CurWageRequire']);
                    $v['difference'] = $v['NeedWageRequire'] - $v['CurWageRequire'];
    ...
    public function getCommentList()
    {
        ...
        // }
        $data = $db->getTableList("[OM_GameOC].[dbo].[T_PlayerComment]", $where, $page, $limit, '*', 'opt_time desc');
        foreach ($data['list'] as $key => &$val) {
            ...
            if (empty($val['admin_name'])) {
                ...
                $val['admin_name'] ='-';
                if(!empty($datarow)){
                    $val['admin_name'] = $datarow['OperatorName'];
                }
            }
        }
        return $this->apiJson($data);
    ...
    public function gameDailyReport()
    {
        switch (input('Action')) {
            case 'list':
                ...
                $userProxyInfo = new UserProxyInfo();
                $ProxyChannelConfig = (new GameOCDB())->getTableObject('T_ProxyChannelConfig')->column('*', 'ProxyChannelId');
                $field = 'a.RoleId,b.ParentID ,ServerID,CONVERT(varchar(30),a.addtime,112) as AddTime,sum(tax) as Tax,sum(GameRoundRunning) as GameRoundRunning,sum(Money) as TotalWin';
                $join = 'left join [CD_UserDB].[dbo].[T_UserProxyInfo] as b on a.RoleID=b.RoleID';
                $group = ' ServerID,CONVERT(varchar(30),a.addtime,112),a.RoleID,b.ParentID';
