...
class Main extends Controller
{
    ...
    public function _initialize()
    {
        ...
        $username = session('merchant_OperatorName');
        if (empty($username)) {
            session('merchant_OperatorName', null);
            session('merchant_OperatorId', null);
            //session(null);
            $this->redirect('merchant/user/login');
        }
...
//                break;
    ...
    protected function getMenu()
    {
        $menu = [];
        $auth = new Auth();
        $auth_rule_list = Db::name('merchant_auth_rule')->where('status', 1)->order('sort,pid asc')->select();
//        echo Db::getlastsql();
//        die();
