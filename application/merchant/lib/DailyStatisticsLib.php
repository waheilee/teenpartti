...
//                  $dsModel::SBWCT_VIP_UP_AWARD   ,   //VIP升级奖励
                ...
                // 活跃用户（当天有在线用户)
                $new_record['active_users'] = $this->DataChangelogsDB->GetDailyActiveUserCountByDay($table1,$day);
                
                // 新增用户（当天注册用户）
                $new_record['new_users'] = $this->AccountDB->GetDailyRegistrCountByDay($day);
    ...
    private function countDailyCoinsConsumptionByChangeType($table, $type=[], $where='') {
        $fields = " ISNULL(sum(GameRoundRunning), 0) as count ";
        $filter = '  WHERE 1=1 ';
        if (!empty($type)) {
            $filter .= " and ChangeType in (" . implode(',', $type) . ")";
        }
