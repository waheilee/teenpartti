...
//            if (!empty($nickname)) $where .= " AND nickname like '%$nickname%' ";
        ...
        $field = "AccountID ID,MachineCode,Mobile,countryCode,AccountName,Locked,LoginName,GmType,RegisterTime,LastLoginIP,LastLoginTime,TotalDeposit,TotalRollOut,Money,RegIP,VipLv,SystemRight,ParentIds,ParentID,OperatorId,ISNULL(ProxyBonus,0) as ProxyBonus,ProxyCommiSwitch,ColorFirstCharge,ColorHand".$ItemVal;
        $file_roleid = 'RoleId';
        if (config('is_portrait') == 1) {
...
//                }
        ...
        $handUserArray = [];
        foreach ($list as &$key){
            if($key['MoneyKey'] == 10210){
                $firstColorMoney += $key['ChangeMoney'];
                if (!in_array($key['RoleId'],$firstUserArray)){
                    $firstUserArray[] = $key['RoleId'];
                }
            }
            if($key['MoneyKey'] == 10211){
                $handColorMoney += $key['ChangeMoney'];
                if (!in_array($key['RoleId'],$handUserArray)){
                    $handUserArray[] = $key['RoleId'];
                }
            }
        }
        $count['firstColorMoney'] = FormatMoney($firstColorMoney);
        $count['handColorMoney'] = FormatMoney($handColorMoney);
        $count['firstUserCount'] = count($firstUserArray);
        $count['handUserCount'] = count($handUserArray);
