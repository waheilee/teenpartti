...
//    }
            ...
            foreach ($list as $k=>&$v){
                switch (intval($v['type'])){
                    case 2:
                        ...
                        $arrfound = array_column($kindlist,'KindID');
                        $foundkey = array_search($v['AdvShareUrl'],$arrfound);
                        if($foundkey!==false){
                            $v['AdvShareUrl']=$kindlist[$foundkey]['KindName'];
                        }
                        break;
                    case 3:
                        $arrFunc = [
    ...
    public function addadvcfg()
    {
        ...
        $this->assign('selectData',$roomlist);
    ...
    public function annouselist()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            if ($status > -1)
                $where['Nullity'] = $status;
            if($stype>-1){
                $where['JumpType'] = $stype;
            }
            $list = $announcement->getList($where, $page, $limit, '*', 'id desc');
            foreach ($list as $k=>&$v){
                switch (intval($v['JumpType'])){
                    case 2:
                        $kindlist =$this->GetKindList();
                        $arrfound = array_column($kindlist,'KindID');
                        $foundkey = array_search($v['AnnouShareUrl'],$arrfound);
                        if($foundkey!==false){
                            $v['AnnouShareUrl']=$kindlist[$foundkey]['KindName'];
                        }
                        break;
                    case 3:
                        $arrFunc = [
            ...
            $count = $announcement->getCount($where);
            return $this->apiReturn(0, $list, '', $count);
        }
        return $this->fetch();
    ...
    public function addannouse()
    {
        $id = intval(input('id', 0));
        $announcement = new Announcement();

        if ($this->request->isAjax()) {
            $AnnouTitle = input('AnnouTitle', '');
            $FuncType = intval(input('FuncType', ''));
            $AnnouPCUrl = input('AnnouPCUrl', '');
            $AnnouUrl = input('AnnouUrl', '');
            $AnnouShareUrl = input('AnnouShareUrl');
            $JumpType = input('JumpType');
            $Sort = input('Sort');
            if ($AnnouTitle == '')
                return $this->apiReturn(100, '', '请填写标题（公告内容）');

            if($FuncType==2){
                if($AnnouUrl=='' || $AnnouPCUrl==''){
                    return $this->apiReturn(100, '', '请上传手机和PC图片');
                }
            }
            $save_data = [
                ...
                'JumpType' =>$JumpType,
                'AnnouShareUrl' =>$AnnouShareUrl,
                'Sort'=>$Sort
            ];
            if ($id > 0) {
                $announcement->updateById($id, $save_data);
            } else
                $announcement->add($save_data);
            $this->synconfig();
            return $this->apiReturn(0, '', '保存成功');
        }
        $data = [
        ...
        if ($id > 0) {
            $data = $announcement->getRowById($id);
        }
        $roomlist= $this->GetKindList();
        $this->assign('selectData',$roomlist);
        $this->assign('info', $data);
    ...
    public function setannouse()
    {
        try {
            if ($this->request->isAjax()) {
                ...
                $status = !$status;
                $announcement = new Announcement();
                $res = $announcement->updateById($id, ['Nullity' => $status]);
                if (!$res) {
                    throw new \Exception('记录更新失败');
                }
            } else {
    ...
    public function delannouse()
    {
        $id = input('id', 0, 'intval');
        $announcement = new Announcement();
        if ($id > 0) {
            $announcement->delRow(['id' => $id]);
            $this->synconfig();
            return $this->apiReturn(0, '', '删除成功');
        } else {
            return $this->apiReturn(100, '', '删除失败');
    ...
    public function giftCardList()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $k => &$v) {
                ...
                $v['StartTime'] = $v['StartTime'] ? date('Y-m-d H:i:s', strtotime($v['StartTime'])) : '';
                $v['EndTime'] = $v['EndTime'] ? date('Y-m-d H:i:s', strtotime($v['EndTime'])) : '';
                $rand = rand(1000,9999);
                $link = $this->encry($rand.'_'.$v['Id']);
                $link = substr($link, 10);
                $v['UrlLink'] = $active_domain.'/?activeid='.$link;
            }
            return $this->apiReturn(0, $list, '', $count);
        }
        return $this->fetch();
    ...
    //加密
    private function encry($str,$key='giftcard123'){
        $str = trim($str);
        ...
        if ($this->request->isAjax()) {
            ...
            if ($data['StartTime'] && $data['EndTime']) {
                if (strtotime($data['StartTime']) > strtotime($data['EndTime'])) {
                    return $this->failJSON('开始时间不能大于结束时间');
                }
            }
            if ($id > 0) {
    ...
    public function  setGiftCardNeedCharge(){

        try {
            if ($this->request->isAjax()) {
                $id = input('id', 0, 'intval');
                $NeedCharge = input('NeedCharge', 0, 'intval');
                if (empty($id)) {
                    throw new \Exception(lang('id不能为空'));
                }
                if ($NeedCharge === 0) {
                ...
                $giftcard = new GiftCardActive();
                $giftcard->updateById($id,['NeedCharge'=>$update_NeedCharge]);
