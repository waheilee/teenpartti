...
//允许携带证书式访问（携带cookie）
    ...
    public function createuser()
    {
        try {
            ...
            save_log('tadagame', '==='.request()->url().'===接口请求数据===' . json_encode($param));
            ...
            if ((strlen($roleid)==7) || in_array($roleid, $test_uidarr)) {
                 $this->config = config('tadagame_test');
            ...
            if (config('is_tadagame_trans') == 1) {
                $post_param['url'] = $url;
                $result = $this->curl(config('trans_url_other').'/tadagame/index/createuser',$post_param);
            } else {
                $result = $this->curl($url);
            }
            save_log('tadagame', '===第三方玩家创建===' . json_encode($result));
            ...
            save_log('tadagame_error', '==='.$ex->getMessage() . $ex->getTraceAsString() . $ex->getLine());
    ...
    public function auth(){
        try {
            ...
            save_log('tadagame', '==='.request()->url().'===接口请求数据===' . json_encode($params));
            $token = $params['token']??'';
            if(empty($token)){
                 $respons = [
                    "errorCode"   =>5,
                ...
                save_log('tadagame', '==='.request()->url().'===响应成功数据===' . json_encode($params));
                return json($respons);
            }
            ...
            save_log('tadagame', '==='.request()->url().'===响应成功数据===' . json_encode($respons));
            return json($respons);
        } catch (Exception $ex) {
            save_log('tadagame_error', '==='.$ex->getMessage() . $ex->getTraceAsString() . $ex->getLine());
            $respons = [
    ...
    public function bet(){
        try {
            ...
            $game_id    = $params['game'];
            if (Redis::get('tadagame_is_exec_bet_'.$transaction_id)) {
                $respons = [
                    "errorCode"   =>1,
            ...
            if ($bet_amount > 0) {
                ...
                $state = $socket->downScore($user_id, $gamemoney, $transaction_id,45000);
                if ($state['iResult']!=0) {
                    $respons = [
                        "errorCode"   =>5,
                        "message"   =>'Other error ',
            ...
            if ($win_amount >= 0) {
                ...
                $state = $socket->UpScore2($user_id, $gamemoney, $transaction_id,45000,$gamemoney2);
            }

            $clear_data = Redis::get('tadagame_game_id_'.$user_id) ?: [];
            $clear_data[$game_id] = 1;
            unset($clear_data[$game_id]);
            if (empty($clear_data)) {
                $state = $socket->ClearLablel($user_id,45000);
                save_log('tadadama', $user_id.'游戏'.$game_id.'打码清零==='.request()->url().'===响应成功数据===' .json_encode($state));
                Redis::rm('tadagame_game_id_'.$user_id);
            } else {
                Redis::set('tadagame_game_id_'.$user_id,$clear_data);
            }
            ...
            Redis::set('tadagame_is_exec_bet_'.$transaction_id,1,3600);
            $balance = $this->getBalance($user_id);
            $respons = [
                "errorCode"   =>0,
                "message"   =>'success',
    ...
    public function cancelBet(){
        try {
            ...
            if (Redis::get('tadagame_is_exec_cancelBet_'.$transaction_id)) {
                $respons = [
                    "errorCode" =>1,
            ...
            if ($win_amount > 0) {
                ...
                if ($bet_amount > 0) {
                    $gamemoney = bcmul($bet_amount,bl,0);
                    $socket = new QuerySocket();
                    $state = $socket->UpScore2($user_id, $gamemoney, $transaction_id,45000,0);
                }
                save_log('tadagame', '===取消下注加钱' . $user_id . '===状态：' . json_encode($state));
            }

            Redis::set('tadagame_is_exec_cancelBet_'.$transaction_id,1,3600);
            $balance = $this->getBalance($user_id);
            $respons = [
                "errorCode"   =>0,
                "message"     =>'success',
    ...
    private function curl($url,$post_data='',$header=[],$type='get') {
        ...
        //执行命令
        $data = curl_exec($curl);
        save_log('tadagame', '==='.request()->url().'===三方返回数据===' . $data);
        //关闭URL请求
        curl_close($curl);
        //显示获得的数据
        return $data;
    ...
    //加密
    private function encry($str,$key='tadagame'){
        $str = trim($str);
    ...
    //解密
    private function decry($str,$key='tadagame'){
