...
class PaySdk
{
    ...
    public function payout($OrderNo,$order,$config=[]){
        ...
        $postdata = [
            ...
            'beneficiaryAccountNumber' =>$pixkey,
            'beneficiaryName'          =>$RealName,
        ...
        $sign_string = $merchant_no.'api.pay'.'payOut'.$timestamp.'SHA512'.json_encode($postdata).$this->secret;
        $sign = hash("sha512", $sign_string);
        $header[] = 'sign:'.$sign;

        $result =$this->curl_post_content($this->api_url,json_encode($postdata), $header);

        save_log('xjpay','post:'.json_encode($postdata).',output:'.$result);
