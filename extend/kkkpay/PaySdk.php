...
class PaySdk
{
    ...
    public function payout($OrderNo,$order,$config=[]){
        ...
        $postdata = [
            ...
            'money'            =>$amount,
            'bankname'         =>'PH_'.trim($order['Province']),
            'accountname'      =>trim($order['RealName']),
            'cardnumber'       =>(int)trim($order['CardNo']),
            'notifyurl'        =>$notify_url,
            'bank_card_remark' =>trim($order['City']),
        ];
        
        $postdata['pay_md5sign'] = $this->createSign($postdata,$this->secret);
        $header = [
            // 'Content-Type: application/x-www-form-urlencoded;charset=utf-8',
        ];

        $result =$this->curl_post_content($this->api_url.'/Payment_Dfpay_add.html',http_build_query($postdata), $header);
        save_log('kkkpay','post:'.json_encode($postdata).',output:'.$result);
