...
class PaySdk
{
    ...
    public function payout($OrderNo,$order,$config=[]){
        ...
        $postdata = [
            ...
            'order_id'          =>$orderId,
            'bank'              =>trim($order['Province']),
            'bank_card_name'    =>trim($order['RealName']),
            'bank_card_account' =>(int)trim($order['CardNo']),
        ...
        $result =$this->curl_post_content($this->api_url.'/api/daifu',http_build_query($postdata), $header);
        save_log('powerpay','post:'.json_encode($postdata).',output:'.$result);
