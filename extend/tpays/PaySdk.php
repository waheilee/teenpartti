...
class PaySdk
{
    ...
    public function payout($OrderNo,$order,$config=[]){
        ...
        $postdata = [
            ...
            'orderId'      =>$orderId,
            'amount'       =>$amount,
            'customName'   =>trim($order['RealName']),
            'customMobile' =>$mobile,
            'customEmail'  =>$email,
        ...
        $postdata['sign'] =$this->createSign($postdata,$config['secret']);

        $header = [
            'application/x-www-form-urlencoded'
        ];
        $result =$this->curl_post_content($this->api_url .'/openApi/payout/createOrder',http_build_query($postdata), $header);

        save_log('tpays','post:'.json_encode($postdata).',output:'.$result);
        ...
        if ($res) {
            ...
            {
                $result['message'] =$res['errorMessages'];
                $result['status'] = false;
            }
        } else {
            $result['system_ref'] = '';
