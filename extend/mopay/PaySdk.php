...
class PaySdk
{
    ...
    public function payout($OrderNo,$order,$config=[]){
        ...
        $postdata = [
            ...
            'mobile'    =>trim($order['City'] ?: $mobile),
            'email'     =>trim($order['Province'] ?: $email),
        ...
        $result =$this->curl_post_content($this->api_url.'/Payment_Dfpay_add.html',$postdata);

        save_log('mopay','post:'.json_encode($postdata).',output:'.$result);
